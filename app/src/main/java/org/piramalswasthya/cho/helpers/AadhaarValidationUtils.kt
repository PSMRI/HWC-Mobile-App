...
object AadhaarValidationUtils {
    ...
    fun isValidAadhaar(aadhaar: String): Boolean {
        if (!aadhaar.matches(Regex("^[2-9]{1}[0-9]{11}$"))) return false

        var checksum = 0
        val reversedDigits = aadhaar.reversed().map { it.toString().toInt() }

        reversedDigits.forEachIndexed { i, digit ->
