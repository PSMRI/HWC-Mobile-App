package  org.piramalswasthya.cho.helpers.addharEditText
...
import org.piramalswasthya.cho.utils.HelperUtil.convertDpToPixel
...
class BlockEditText : FrameLayout {
    ...
    private val editTexts = SparseArray<EditText?>()
    ...
    private var cardIconSize = convertDpToPixel(48f,context).toInt()
    ...
    private var isToggleAdded:Boolean = false
    private var passwordToggleSize = resources.getDimensionPixelSize(R.dimen.password_toggle_size)
    private var passwordToggleColor = resources.getColor(android.R.color.black)
    ...
    private fun init(context: Context, attrs: AttributeSet?) {
        ...
        hint = a.getString(R.styleable.BlockEditText_hint1)
        setHint(hint)
        ...
        inputType = a.getInt(
            R.styleable.BlockEditText_inputType1,
            inputType
        )
        ...
        while (i < noOfBlock) {
            ...
            if (editTexts[i] == null) {
                editText = if (editTextStyle == -1) {
                    ...
                    EditText(context, null, editTextStyle)
                }
                    ...
                    if (hintTextView != null) {
                        val hintColorFocus: ColorStateList = ColorStateList.valueOf(
                            editText.textColors.getColorForState(
                                ...
                                ContextCompat.getColor(context, R.color.seed)
                            ))
            ...
            if (i + 1 < noOfBlock && separator != null) {
                ...
                textView.setTextAppearance(separatorTextAppearance)
    ...
    private fun setEditTextEnable(editText: EditText?, i: Int) {
    ...
    private fun createKeyListener(editText: EditText, i: Int): OnKeyListener {
    ...
    private fun setCustomInsertionActionModeCallback(editText: EditText?, callback: ActionMode.Callback?) {
    ...
    private fun setTextSize(editText: EditText?, textSize: Float) {
    ...
    private fun setEdiTextBackground(editText: EditText?, drawable: Drawable?) {
...
//            )
    ...
    fun setHintTextAppearance(textAppearance: Int) {
        ...
        if (hintTextView != null) {
            hintTextView!!.setTextAppearance(textAppearance)
        }
    }
...
//    }
    ...
    private fun createActionModeCallback(editText: EditText): ActionMode.Callback {
    ...
    private fun createTextChangeListener(editText: EditText, index: Int): TextWatcher {
    ...
    inner class LengthFilter(private val editText: EditText, private val index: Int) : InputFilter {
