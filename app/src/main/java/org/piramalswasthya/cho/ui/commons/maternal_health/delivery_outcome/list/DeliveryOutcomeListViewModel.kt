...
class DeliveryOutcomeListViewModel @Inject constructor(
    ...
    private val allPatientList = patientRepo.getDeliveredWomenList()
    ...
    val patientList = allPatientList.combine(filter) { list, filterText ->
        if (filterText.isEmpty()) {
            ...
            list.filter { patient ->
                patient.patient.firstName?.contains(filterText, ignoreCase = true) == true ||
                patient.patient.lastName?.contains(filterText, ignoreCase = true) == true ||
