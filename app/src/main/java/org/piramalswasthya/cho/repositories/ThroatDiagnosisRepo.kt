...
class ThroatDiagnosisRepo @Inject constructor(
    ...
    suspend fun processThroatVisits(): Boolean {
        return withContext(Dispatchers.IO) {
            ...
            val networkList = mutableListOf<ThroatDiagnosisNetwork>()
            unsyncedList.forEach { assessment ->
                ...
                if (patient?.beneficiaryID != null && patient.beneficiaryRegID != null) {
                    networkList.add(ThroatDiagnosisNetwork(patient.beneficiaryID.toString(), patient.beneficiaryRegID.toString(), assessment))
                }
            }
            ...
            try {
                val response = amritApiService.postThroatForm(networkList)
                    ...
                    if (responseString != null) {
                        ...
                        if (jsonObj.optInt("statusCode") == 200) {
                            unsyncedList.forEach {
                                ...
                                throatDiagnosisAssessmentDao.update(it)
                            }
    ...
    suspend fun pullThroatVisitsFromServer(): Boolean {
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                return@withContext pullThroatVisitsFromServer()
            } catch (e: Exception) {
