...
class InfantRegRepo @Inject constructor(
                ...
                try {
                    ...
                    val merged = if (existing != null) {
                        if (existing.syncState == SyncState.SYNCED) {
                            incoming.copy(
                                id = existing.id,
                                childPatientID = incoming.childPatientID ?: existing.childPatientID,
                                babyName = incoming.babyName ?: existing.babyName,
                                infantTerm = incoming.infantTerm ?: existing.infantTerm,
                                corticosteroidGiven = incoming.corticosteroidGiven ?: existing.corticosteroidGiven,
                                genderID = incoming.genderID ?: existing.genderID,
                                babyCriedAtBirth = incoming.babyCriedAtBirth ?: existing.babyCriedAtBirth,
                                resuscitation = incoming.resuscitation ?: existing.resuscitation,
                                referred = incoming.referred ?: existing.referred,
                                hadBirthDefect = incoming.hadBirthDefect ?: existing.hadBirthDefect,
                                birthDefect = incoming.birthDefect ?: existing.birthDefect,
                                otherDefect = incoming.otherDefect ?: existing.otherDefect,
                                weight = incoming.weight ?: existing.weight,
                                breastFeedingStarted = incoming.breastFeedingStarted ?: existing.breastFeedingStarted,
                                opv0Dose = incoming.opv0Dose ?: existing.opv0Dose,
                                bcgDose = incoming.bcgDose ?: existing.bcgDose,
                                hepBDose = incoming.hepBDose ?: existing.hepBDose,
                                vitkDose = incoming.vitkDose ?: existing.vitkDose,
                                outcomeAtBirth = incoming.outcomeAtBirth ?: existing.outcomeAtBirth,
                                typeOfResuscitation = incoming.typeOfResuscitation ?: existing.typeOfResuscitation,
                                newbornComplications = incoming.newbornComplications ?: existing.newbornComplications,
                                currentStatusOfBaby = incoming.currentStatusOfBaby ?: existing.currentStatusOfBaby,
                                ...
                                birthDoseVaccinesGiven = incoming.birthDoseVaccinesGiven ?: existing.birthDoseVaccinesGiven,
                                reasonForNoVaccines = incoming.reasonForNoVaccines ?: existing.reasonForNoVaccines,
                                vitaminKInjectionGiven = incoming.vitaminKInjectionGiven ?: existing.vitaminKInjectionGiven,
                                reasonForNoVitaminK = incoming.reasonForNoVitaminK ?: existing.reasonForNoVitaminK,
                                birthCertificateIssued = incoming.birthCertificateIssued ?: existing.birthCertificateIssued,
    ...
    private fun mergeChildDownsync(
        ...
        val incoming = InfantRegCache(
            ...
            outcomeAtBirth = existing?.outcomeAtBirth,
            typeOfResuscitation = existing?.typeOfResuscitation,
            newbornComplications = existing?.newbornComplications,
            currentStatusOfBaby = existing?.currentStatusOfBaby,
            causeOfDeath = existing?.causeOfDeath,
            otherCauseOfDeath = existing?.otherCauseOfDeath,
            birthDoseVaccinesGiven = existing?.birthDoseVaccinesGiven,
            reasonForNoVaccines = existing?.reasonForNoVaccines,
            vitaminKInjectionGiven = existing?.vitaminKInjectionGiven,
            reasonForNoVitaminK = existing?.reasonForNoVitaminK,
            birthCertificateIssued = existing?.birthCertificateIssued,
            processed = "C",
