    ...
    suspend fun processElderlyVisits(): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                ...
                val response = amritApiService.postElderlyForm(payload.map { it.second })
                ...
                Timber.e(e, "Error syncing Elderly Health records")
                false
            }
        }
    }
        ...
        return withContext(Dispatchers.IO) {
            ...
            repeat(MAX_PULL_ATTEMPTS) { attempt ->
                try {
                    ...
                    val response = amritApiService.getElderlyVisits(villageList)
                    ...
                    when (jsonObj.getInt("statusCode")) {
                        STATUS_OK -> {
                            ...
                            if (dataArray != null) {
                                upsertPulledElderlyData(dataArray)
                                return@withContext true
                            }
                        }
                        STATUS_TOKEN_EXPIRED -> {
                            ...
                            if (!tokenRefreshed) {
                                Timber.w("Token refresh failed while pulling Elderly records")
                            ...
                            Timber.w("Max retry attempts reached while pulling Elderly records")
                            return@withContext false
                        }
                    ...
                    Timber.e(e, "Socket timeout while pulling Elderly records after retries")
                    return@withContext false
                } catch (e: Exception) {
                    Timber.e(e, "Error pulling Elderly records")
                    return@withContext false
                }
                return@withContext false
            }
